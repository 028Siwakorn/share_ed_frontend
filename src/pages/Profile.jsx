...
const MOCK_MY_POSTS = [
  { id: 10, title: 'สรุปแกรมม่า Tense พื้นฐาน', author: 'Somchai Learner', level: 'มัธยมศึกษาตอนต้น', subject: 'ภาษาอังกฤษ', likes: 25, views: '150', image: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=500&q=80', status: 'ACTIVE' },
];

const MOCK_DRAFTS = [
  { id: 11, title: 'ฟิสิกส์ การเคลื่อนที่แนวตรง (ยังไม่เสร็จ)', author: 'Somchai Learner', level: 'มัธยมศึกษาตอนปลาย', subject: 'ฟิสิกส์', likes: 0, views: '0', image: 'https://images.unsplash.com/photo-1636466497217-26a8cbeaf0aa?w=500&q=80', status: 'DRAFT' },
];
...
const MOCK_MILESTONES = [
  { 
    ...
    description: 'มีผู้ติดตามครบ 10 คนแล้ว',
    type: 'FOLLOWERS', 
    current: 10, 
    target: 10,
    status: 'READY_TO_CLAIM' // LOCKED, READY_TO_CLAIM, CLAIMED
  },
  { 
    id: 2, 
    title: 'Content Loved', 
    description: 'มีคนกดไลค์ให้ครบ 50 ไลค์แล้ว',
    type: 'LIKES', 
    current: 45, 
    target: 50,
    status: 'LOCKED'
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 p-6 sm:p-10 mb-8">
          <div className="flex flex-col sm:flex-row gap-8 items-start sm:items-end">
            ...
            <div className="flex-1 pb-2">
              <h1 className="text-3xl font-extrabold text-slate-900">Somchai Learner</h1>
              <p className="text-slate-500 font-medium text-lg mb-4">@somchai_ed</p>
        ...
        <div className="min-h-[400px]">
          {activeTab === 'posts' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {MOCK_MY_POSTS.map(post => <PostCard key={post.id} post={post} viewMode="grid" />)}
            </div>
          )}

          {activeTab === 'drafts' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {MOCK_DRAFTS.map(post => (
